...
calc = Calculator(operation_type="multiply")
...
def test_infinity_positive(num_a, num_b):
    assert calc.operation(number_a=num_a, number_b=num_b) == "∞"


@pytest.mark.parametrize(
    "num_a, num_b",
    list_infinity_negative)
def test_infinity_negative(num_a, num_b):
    assert calc.operation(number_a=num_a, number_b=num_b) == "-∞"
